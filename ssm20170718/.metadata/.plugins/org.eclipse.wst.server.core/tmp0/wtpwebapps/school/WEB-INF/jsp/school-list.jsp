	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<c:forEach items="${list}" var="s">
			<tr class="text-c">
				...
				<td class="td-status"><span class="label <c:choose><c:when test="${s.status}">label-success</c:when><c:otherwise>label-defaunt</c:otherwise></c:choose> radius"><c:choose><c:when test="${s.status}">已启用</c:when><c:otherwise>已停用</c:otherwise></c:choose></span></td>
				<td class="td-manage">
				<c:choose><c:when test="${s.status}">
					<a style="text-decoration:none" onClick="member_stop(this,'${s.id}')" href="javascript:;" title="停用">
						<i class="Hui-iconfont">&#xe631;</i>
					</a>
				</c:when><c:otherwise>
					<a style="text-decoration:none" onClick="member_start(this,'${s.id}')" href="javascript:;" title="启用">
						<i class="Hui-iconfont">&#xe6e1;</i>
					</a>
				</c:otherwise>
				</c:choose>
